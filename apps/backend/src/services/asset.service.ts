...
export async function confirmAsset(
    ...
    if (!creator) {
        ...
        creator = await prisma.creator.create({
            data: {
                walletAddress,
                displayName: `Creator ${shortAddr}`,
                bio: "Creator on OriginChain",
...
/**
 * Dev-mode asset registration — skips on-chain transaction verification.
 * Creates the asset directly in the database with a mock tx hash.
...
export async function devRegisterAsset(
    ...
    // Generate a mock tx hash for dev mode
    const mockTxHash = "0x" + createHash("sha256").update(`dev-${contentHash}-${Date.now()}`).digest("hex") as `0x${string}`;
    ...
    // Persist tags
    const rawTags = finalMetadata.tags ?? [];
    const normalizedTags = rawTags.map((t) => t.trim().toLowerCase()).filter((t) => t.length > 0);
    if (normalizedTags.length > 0) {
        await Promise.all(
            normalizedTags.map(async (name) => {
