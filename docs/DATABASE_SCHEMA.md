...
> [!NOTE]
> The ERD reflects the **implemented** schema only. Planned-but-unimplemented tables (REPUTATION_SCORES, ANALYTICS_EVENTS) are documented separately in the Table Definitions section below.
...
|---|---|---|
| id | uuid | PK, default gen_random_uuid() |
| name | varchar(50) | unique, not null — normalized to lowercase on write |

Stores each unique tag name once. The `name` unique constraint is the deduplication key — `upsert` on `name` guarantees no duplicate rows even if two assets share a tag.

### `asset_tags`
| Column | Type | Constraints |
|---|---|---|
| asset_id | uuid | FK → assets.id, part of composite PK |
| tag_id | uuid | FK → tags.id, part of composite PK |

Many-to-many join table linking assets to their tags. Composite PK `(asset_id, tag_id)` prevents duplicate join rows. Tags are written at `POST /assets/confirm` time from `finalMetadata.tags` — the same array already pinned to IPFS — so no additional AI call or user input is required at tag-persist time.

> [!NOTE]
> Tag names are stored **lowercase** (trimmed + `toLowerCase()` applied before write). Case-insensitive matching in `GET /search` uses Prisma's `mode: "insensitive"` against the lowercase-stored name, so queries for `"Photography"`, `"photography"`, and `"PHOTOGRAPHY"` all match identically.

## Prisma Model Naming Convention

All seven models follow the same pattern: Prisma model name is PascalCase singular, actual Postgres table name is snake_case plural (enforced via `@@map`):
