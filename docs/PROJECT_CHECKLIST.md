...
- [x] Build Blockchain Service (`services/blockchain/`): contract registry, read wrapper, retries, event decoder, error translation, chunked `getLogs` for large block ranges (read-only scope, per design decision — this backend never sends transactions itself). Migrated: `getCreatorReputation`, `creator-indexer.ts`, `asset-indexer.ts`. Migrating `asset-indexer.ts` surfaced and fixed a real pre-existing bug: it had never advanced past its deployment block since inception (unbounded `getLogs` range rejected by the RPC provider, failing silently every poll) — the asset orphan-recovery safety net had likely been non-functional for a long stretch, undetected since the primary `/assets/confirm` path doesn't depend on it. Now fixed and verified genuinely caught up to real chain tip via three independent non-circular methods. Remaining: full migration of `asset.service.ts`/`review.service.ts`/`organization.service.ts`
